...
export default function FeesFilterBar() {
  ...
  const updateParams = (newClass = classLevel, newQuery = query) => {
  ...
  return (
    <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
      ...
      <form onSubmit={handleSearch} className="relative w-full md:max-w-xs flex gap-2">
        ...
        <button
          type="submit"
          className="px-3 py-2 bg-primary text-white text-xs font-semibold rounded-xl hover:bg-primary-800 transition-colors shadow-sm"
        >
          Search
        </button>
      </form>
